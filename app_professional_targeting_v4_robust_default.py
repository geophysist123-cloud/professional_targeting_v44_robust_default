...
#  11) Robust Z-score / Percentile / Winsorized Min-Max normalization
...
def minmax_score(series):
    ...
    return winsorized_minmax_score(series)
...
def calculate_data_confidence(df):
    """
    Data confidence is based strictly on availability of the two
    geophysical inputs.

        Magnetic + Gravity available -> 100%
        Magnetic only                -> 60%
        Gravity only                 -> 40%
        Neither available            -> No Score (NaN)
    """
    mag_present = _numeric_series(
        df.get("Mag_Anomaly", pd.Series(index=df.index, dtype="float64"))
    ).notna()
    grav_present = _numeric_series(
        df.get("Grav_Anomaly", pd.Series(index=df.index, dtype="float64"))
    ).notna()
...
# Final-model constants. Keeping these explicit makes the scoring model
# auditable and prevents hidden hard-coded percentages elsewhere in the app.
...
def calculate_target_scores(
    ...
    MODEL:
      1. Normalize |Magnetic| and |Gravity| to 0..100.
      2. Apply user-defined Magnetic/Gravity weights.
      3. Apply Data Confidence:
           100% = both signals available
            60% = Magnetic only
            40% = Gravity only
         No Score = neither available
      4. Calculate Magnetic and Gravity contributions.
      5. Calculate Concordance ONLY when both signals are available.
      6. When both signals exist:
           Target Score = 85% Evidence + 15% Concordance
         When only one signal exists:
           Target Score = available Evidence score
         When neither exists:
    ...
    # 1. NORMALIZATION
    # --------------------------------------------------------
    # Robust Z-score is the professional default/recommended method.
    ...
    # Preserve the existing database/export column names.
    ...
    mag_weight_factor = magnetic_weight / total_weight
    grav_weight_factor = gravity_weight / total_weight

    # --------------------------------------------------------
    # 3. DATA CONFIDENCE
    # --------------------------------------------------------
    result["Data_Confidence"] = calculate_data_confidence(result)
    confidence_factor = result["Data_Confidence"] / 100.0

    # --------------------------------------------------------
    # 4. CONTRIBUTIONS
    # --------------------------------------------------------
    # Missing individual signals contribute zero. Confidence then reduces
    # the available evidence according to the explicit 100/60/40 rules.
    result["Mag_Contribution"] = (
        result["Mag_Normalized"].fillna(0.0)
        * mag_weight_factor
        * confidence_factor.fillna(0.0)
    )
    ...
    # 5. CONCORDANCE
    # --------------------------------------------------------
    # Concordance has meaning only when BOTH independent signals exist.
    both = (
        result["Mag_Normalized"].notna()
        & result["Grav_Normalized"].notna()
    )
    ...
    # --------------------------------------------------------
    # 6. FINAL TARGET SCORE
    # --------------------------------------------------------
    # Both signals: 85% evidence + 15% concordance.
    result["Target_Score"] = float("nan")
    result.loc[both, "Target_Score"] = (
        core_score.loc[both] * (EVIDENCE_WEIGHT / 100.0)
        + result.loc[both, "Concordance"] * (CONCORDANCE_WEIGHT / 100.0)
    )

    # One signal: do not invent a Concordance value and do not waste the
    # unavailable 15% of the score. The confidence-adjusted evidence is the
    # final score for a single-signal observation.
    one_signal = (
        result["Data_Confidence"].eq(60.0)
        | result["Data_Confidence"].eq(40.0)
    )
    result.loc[one_signal, "Target_Score"] = core_score.loc[one_signal]

    # No magnetic and no gravity -> explicitly No Score.
...
# ============================================================
    ...
    if df_targets.empty:
        ...
        st.subheader("⚖️ Weighting, Data Confidence & Concordance")
        st.caption(
            "Scoring model: 85% weighted geophysical evidence + 15% magnetic–gravity concordance when both datasets are available. "
            "Single-signal records use confidence-adjusted evidence only; records with neither signal receive No Score."
        )
